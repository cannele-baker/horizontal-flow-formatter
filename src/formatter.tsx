...
export interface Data {
    ...
    answer: string
    questions: string
    prompts: { qna_id: number; display_text: string, display_order?: number, metadata?: { [key: string]: any } }
}
...
export const createNode = (rowNum: number, parent: number, data?: Data): Node => {
    if (data) {
...
export const insertNode = (node2dArr: Node[][], rowNum: number, colNum: number, parent: number, data?: Data): Node[][] => {
...
export const createNode2dArr = (
    ...
    if (searched) {
        if (Array.isArray(searched?.prompts) && searched.prompts.length > 0) {
            node2dArr = insertNode(node2dArr, rowNum, colNum, parent ?? searched.qna_id, searched)

            searched.prompts.forEach((prompt, index) => {
                node2dArr = createNode2dArr(data, node2dArr, prompt['qna_id'], rowNum + index, colNum + 1, searched.qna_id,)
            })
        } else {
